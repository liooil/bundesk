 ...
 *   1. CLI flag: `--env=development` / `--env=production` (or `--env <value>`)
 ...
 * `--env=staging` or a NODE_ENV=staging falls through untouched (the flag
...
/** Extracts `--env=...` / `--env ...` from argv; invalid values are ignored. */
